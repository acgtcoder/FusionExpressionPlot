...
(4) Get list of samples in cohort
...
(2) In R with this as the working directory
    ...
    1. Load code
            ...
            library( "ExonExpressionPlot" )

    2. Set up parameters for running
    
      One convention used below is to save the results of any computation
      that takes more than a couple of seconds, or that represents a reasonably
      independent data object used as a parameter elsewhere. These are saved
      as .RDS files.
...
3. Set up the data formats used for processing gene expression files
        ...
        sampleExpressionFilesDF <- getCohortFiles( sampleExpressionFileMap, cohortSamples )
...
1. Set up for normalization
        ...
        cohortExpression.RDS <- "cohortExpression.RDS"
        normalizedCohortExpression.RDS <- "normalizedCohortExpression.RDS"
...
===================
    ...
    fusionData.RDS <- "fusionData.RDS"
    selectedFusions.RDS <- "selectedFusions.RDS"
    source("cohortFusions.R")
    source("cohortNormalizedExpression.R")
    fusionDF <- getMapSpliceCohortFusionData( fusionFile );
    saveRDS( fusionData, file=fusionData.RDS )
    ...
    saveRDS( selectedFusions, file=selectedFusions.RDS )
...
==========
    ...
    selectedFusions <- readRDS( selectedFusions.RDS )
    normalizedCohortExpression <- readRDS( normalizedCohortExpression.RDS )

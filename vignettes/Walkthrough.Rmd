...
:   Chromosomes are named with a prefixed "chr" and then the chromosome number
    ...
    with the promolgated genome. There should never be mixed names for the
...
:   Features are specified by a location on the genome. This location is
    ...
    (inclusively). Features on the - (3' to 5' strand) actually run in the
...
:   To allow for future additions to data files, column headings are used to 
    ...
    insensitively. Anything characters other than letters, digits, periods,
...
:   The gene annotation file (gaf) reference used with the TCGA RNASeq data as
    ...
    transcript names exon models.

Exon expression files (bt.exon_quantification.txt)
  
:   This file describes RNA expression for one sample on a per-exon basis. It is
    normally downloaded from the TCGA website and is publically available after a
    short embargo period. It is usally called bt.exon_quantification.txt, with the
    directory it is in specifying the sample this file is associated with.
...
include the SNP annotations is faster to extract. It can be downloaded from
